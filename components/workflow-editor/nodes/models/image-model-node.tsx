        ...
        <NodeContextMenu nodeId={id} type="context">
            <Card className={`relative w-[340px] bg-[#1a1a1a] border-zinc-800 transition-all ${selected ? 'ring-2 ring-primary shadow-2xl scale-[1.02]' : 'shadow-lg'}`}>
                <div className="p-0 overflow-hidden rounded-xl max-h-[600px] flex flex-col">
                    ...
                    {/* Preview Area */}
                    <div className="relative aspect-square w-full bg-[#121212] flex items-center justify-center group flex-shrink-0">
                        ...
                        {imageUrl ? (
                            <div className="relative w-full h-full p-1 animate-in fade-in duration-500">
                                <img
                                    src={imageUrl}
                ...
                <div className="absolute left-0 top-0 bottom-0 flex flex-col pointer-events-none">
                    {inputs.map((input, index) => (
                        ...
                        >
                            <div
                                className={`absolute right-full mr-3 flex items-center gap-2 text-[11px] font-bold tracking-tight uppercase whitespace-nowrap transition-opacity ${getLabelClass(input.type)} ${!imageUrl && !selected ? 'opacity-40' : 'opacity-100'}`}
                            >
                ...
                <div className="absolute right-0 top-0 bottom-0 flex flex-col pointer-events-none">
                    {outputs.map((output, index) => (
                        ...
                        >
                            <div
                                className={`absolute left-full ml-3 flex items-center gap-2 text-[11px] font-bold tracking-tight uppercase whitespace-nowrap transition-opacity ${getLabelClass(output.type)} ${!imageUrl && !selected ? 'opacity-40' : 'opacity-100'}`}
                            >
                                {output.label}
                            </div>
                            <Handle
